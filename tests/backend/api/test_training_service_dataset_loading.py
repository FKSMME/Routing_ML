...
def test_load_dataset_reads_csv(training_service: TrainingService, tmp_path: Path) -> None:
    ...
    cfg = DataSourceConfig(offline_dataset_path=str(csv_path))
...
def test_load_dataset_reads_parquet(monkeypatch: pytest.MonkeyPatch, training_service: TrainingService, tmp_path: Path) -> None:
    ...
    cfg = DataSourceConfig(offline_dataset_path=str(parquet_path))
    result = training_service._load_dataset(cfg)

    pd.testing.assert_frame_equal(result, expected)


def test_load_dataset_uses_mssql_when_offline_file_missing(monkeypatch: pytest.MonkeyPatch, training_service: TrainingService) -> None:
    expected = pd.DataFrame({"ITEM_CD": ["C"], "ITEM_NM": ["Gamma"]})

    monkeypatch.setattr(database, "fetch_item_master", lambda columns=None: expected)

    cfg = DataSourceConfig(offline_dataset_path="/non/existent/path.csv")
    result = training_service._load_dataset(cfg)
