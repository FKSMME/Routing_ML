...
- [ ] `http://10.204.2.28:8000/api/workflow/graph`에 접속해 JSON 구조가 보이는지 확인한다. 최신 로그: `deliverables/onboarding_evidence/api_health_corpnet.log`. 재검증 계획: 2025-10-07 이전에 VPN이 활성화된 사내 QA PC에서 IT 네트워크 팀과 함께 브라우저 및 `curl` 호출을 재수행한다.
