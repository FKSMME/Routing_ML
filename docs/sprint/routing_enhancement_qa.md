...
- 오프라인 IndexedDB 저장본 → 온라인 백엔드 플러시 경로 자동화: `frontend/tests/unit/indexedDbPersistence.flush.test.ts`, `frontend/tests/e2e/routing-offline-sync.spec.ts`, `tests/backend/api/test_routing_groups_snapshots.py` 로 큐 플러시·버전 병합 케이스 검증. 【F:frontend/tests/unit/indexedDbPersistence.flush.test.ts†L1-L85】【F:frontend/tests/e2e/routing-offline-sync.spec.ts†L1-L91】【F:tests/backend/api/test_routing_groups_snapshots.py†L1-L149】
