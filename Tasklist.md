...
3. 예측 서비스(predictor) — 상세 문서: `task_details/stage3_detail.md`
 ...
 - [ ] (설계) 모듈/함수 상호의존 그래프 API 요구사항 정의 및 데이터 스키마 초안 작성 — `docs/graph_workflow_ui_plan.md`
 - [ ] (구현) 그래프 노드 메타데이터/설정값 제공용 `/api/workflow/graph` 설계 반영 — `docs/graph_workflow_ui_plan.md`
...
4. 프런트엔드(React, B안) — 상세 문서: `task_details/stage4_detail.md`
 ...
 - [ ] (설계) 블루스크린형 워크플로우 그래프 UI 시나리오, 노드 상호작용 플로우, 더블클릭 설정 팝업 UX 정의 — `docs/graph_workflow_ui_plan.md`
 - [ ] (구현) 그래프 렌더링 라이브러리 선택(Dagre/Cytoscape 등) 및 컴포넌트 구조 설계 — `docs/graph_workflow_ui_plan.md`
 - [ ] (구현) 더블클릭 설정 패널 상태관리 및 설정 편집 UX 설계 — `docs/graph_workflow_ui_plan.md`
 - [ ] (테스트) 그래프 상호작용(드래그, 확대/축소, 팝업) 테스트 시나리오 수립 — `docs/graph_workflow_ui_plan.md`
